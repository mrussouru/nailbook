...
export default function GastosSalon() {
  ...
  async function cargarGastos() {
    ...
    if (error) {
      console.error("Error cargando gastos del salón:", error);
  ...
  async function guardarGasto(e) {
    ...
    const consulta = editandoId
      ? supabase.from("gastos_salon").update(payload).eq("id", editandoId)
      : supabase.from("gastos_salon").insert(payload);

    const { error } = await consulta;

    if (error) {
      console.error("Error guardando gasto del salón:", error);
  ...
  async function eliminarGasto(gasto) {
    ...
    if (error) {
      console.error("Error eliminando gasto del salón:", error);
  ...
  const resumen = useMemo(() => {
    ...
    return {
      total,
      publicidad: porCategoria["Publicidad general"] || 0,
      categorias
    };
  }, [gastos]);
    ...
    <div>
      <div style={{ marginBottom: 22 }}>
        ...
        <p style={{ margin: 0, color: "#888", fontSize: 14 }}>
          Registrá aquí alquiler, servicios y otros gastos generales del negocio. Los gastos propios de Tamy se administran por separado.
        </p>
      </div>
        ...
        >
          ...
          <Campo label="Concepto">
            <input
              ...
              placeholder="Ej: Alquiler del local, factura de luz..."
              style={inputStyle}
            />
          </Campo>
      ...
      >
        ...
        <div style={{ marginTop: 14, color: "#777", fontSize: 13 }}>
          Publicidad general: <strong>${dinero(resumen.publicidad)}</strong>
        </div>
      </div>
